...
REPLAY_JSON_SCHEMA = "blender_tetris.replay"
    ...
    if isinstance(raw, list):
        for item in raw:
            ...
            if isinstance(item, dict):
                ...
                elif len(item) == 2 and isinstance(item[0], (list, tuple)) and len(item[0]) == 2:
                    (x, z), key = item
            ...
            try:
                ...
                raise ValueError("Replay JSON occupied contains invalid coordinates") from exc
...
def _parse_current(raw: Any) -> tuple[str, int, int, int] | None:
    ...
    if isinstance(raw, dict):
        key = raw.get("key") if "key" in raw else (raw.get("piece") if "piece" in raw else raw.get("type"))
...
def _parse_step(raw: Any) -> RecordedState:
    ...
    if isinstance(raw, dict):
        ...
        return RecordedState(occupied=occupied, current=current, game_over=game_over, reason=reason)
...
def parse_replay_json(payload: Any) -> tuple[list[RecordedState], dict[str, Any], int]:
    ...
    doc: Any = payload
    if isinstance(payload, dict) and "blender_tetris_replay" in payload:
    ...
    if version >= 2:
        ...
        for step_raw in steps_raw:
            ...
            state = RecordedState(occupied=occupied, current=current, game_over=game_over, reason=reason)

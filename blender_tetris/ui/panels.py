"""侧边栏 UI 面板（View3D > Sidebar > Tetris）。
...
def _format_keybind(*, key: str, shift: bool, ctrl: bool, alt: bool, oskey: bool) -> str:
    if not key:
        return "(Unbound)"
...
def _draw_keybind_row(*, layout: bpy.types.UILayout, settings, label: str, prefix: str) -> None:
    ...
    row.label(text=_format_keybind(key=key, shift=shift, ctrl=ctrl, alt=alt, oskey=oskey))
...
class BLTETRIS_PT_main(bpy.types.Panel):
    ...
    def draw(self, context: bpy.types.Context):
        ...
        if sess is not None:
            ...
            speed = float(getattr(sess, "drop_interval", float(getattr(settings, "drop_interval", 0.0))))
        ...
        if sess is not None:
            ...
            if callable(peek_next):
                next_keys = peek_next(count=int(getattr(settings, "next_queue_size", 5)))
                row.label(text=' '.join(next_keys) if next_keys else "-", translate=False)
        ...
        if bool(settings.ui_show_advanced):
            ...
            ply2_col.enabled = str(getattr(settings, "ai_ply", "PLY1") or "PLY1") == "PLY2"
            ply2_col.prop(settings, "ai_ply2_next_weight")

            ai_settings.prop(settings, "ai_strategy")

            weights_box = ai_settings.box()
            weights_box.enabled = str(getattr(settings, "ai_strategy", "STABLE") or "STABLE") == "CUSTOM"
            ...
            drop_col.enabled = str(getattr(settings, "ai_drop_mode", "HARD") or "HARD") == "SOFT"
            ...
            keybind_box.label(text="Click Set, then press a key (RMB to cancel)", translate=True)
            keybind_box.enabled = not state.running

            _draw_keybind_row(layout=keybind_box, settings=settings, label="Move Left", prefix="kb_move_left")
            _draw_keybind_row(layout=keybind_box, settings=settings, label="Move Right", prefix="kb_move_right")
            _draw_keybind_row(layout=keybind_box, settings=settings, label="Soft Drop", prefix="kb_soft_drop")

            keybind_box.separator()
            _draw_keybind_row(layout=keybind_box, settings=settings, label="Rotate CW", prefix="kb_rotate_cw")
            _draw_keybind_row(layout=keybind_box, settings=settings, label="Rotate CW (Alt)", prefix="kb_rotate_cw_alt")
            _draw_keybind_row(layout=keybind_box, settings=settings, label="Rotate CCW", prefix="kb_rotate_ccw")
            _draw_keybind_row(layout=keybind_box, settings=settings, label="Hold", prefix="kb_hold")
            _draw_keybind_row(layout=keybind_box, settings=settings, label="Hard Drop", prefix="kb_hard_drop")
            _draw_keybind_row(layout=keybind_box, settings=settings, label="Pause", prefix="kb_pause")

            keybind_box.separator()
            keybind_box.operator("bltetris.reset_keybinds", text="Reset Keybinds", translate=True)
            ...
            if str(getattr(settings, "speed_curve", "MULTIPLIER") or "MULTIPLIER") == "MULTIPLIER":
            ...
            combo_col.enabled = str(getattr(settings, "combo_scoring_mode", "MULTIPLY") or "MULTIPLY") == "MULTIPLY"
            ...
            recording_on = bool(sess is not None and getattr(sess, "recording_active", False))
            recorded_steps = int(len(getattr(sess, "recording", [])) if sess is not None else 0)
